...
const WeaponFilters = () => {
    ...
    const handleWeaponFilter = useCallback(
        async (weapon: string) => {
            if (weapon === "Melee") {
                try {
                    const response = await fetch(
    ...
    }, [searchParams, handleWeaponFilter]); // Add searchParams as dependency
                    ...
                    {meleeSkins.map(
                        (skin: SkinCardProps) =>
                            ...
                            !skin.displayName.includes("Random Favorite") && (
                                <div
                                    className="hover:scale-105 hover:opacity-50 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 p-2"
                                    key={skin.uuid}
                                >
                                    <SkinCard
                                        uuid={skin.uuid}
                                        displayName={
                                            skin.displayName || "Unknown"
                                        }
